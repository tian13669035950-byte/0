...
/** Move mouse along a curved path then click — avoids the "teleport" tell */
async function humanClick(page: Page, selector: string) {
  ...
  // Random landing point inside element (avoid exact center)
  const tx = box.x + box.width  * (0.25 + Math.random() * 0.5);
  const ty = box.y + box.height * (0.25 + Math.random() * 0.5);

  // Random starting point somewhere on the page above/beside the element
  const sx = Math.max(10, tx - 200 + Math.random() * 400);
  const sy = Math.max(10, ty - 150 + Math.random() * 100);

  // Move through 4-7 intermediate points with slight jitter
  ...
  // Brief "hover" pause before pressing — humans read before they click
...
async function humanType(page: Page, selector: string, text: string) {
  ...
  // Select-all then delete existing content
  await page.keyboard.press("Control+a");
  await page.keyboard.press("Delete");
  await page.waitForTimeout(80 + Math.random() * 80);
  for (const ch of text) {
    await page.keyboard.type(ch);
    // Occasional longer pause simulating thinking (every ~8 chars)
    const delay = Math.random() < 0.12
      ? 200 + Math.random() * 300   // thinking pause
      : 40  + Math.random() * 60;   // normal keystroke
  ...
  try {
    ...
    for (let i = 0; i < effectiveSteps.length; i++) {
      ...
      if (step.type === "listen") {
        ...
        try {
          await page.waitForSelector(selector, { timeout: 8000 });
          // Move mouse naturally first, then right-click at that position
          const el = await page.$(selector);
